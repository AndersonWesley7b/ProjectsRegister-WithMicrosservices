...
public sealed class UsersRepository : IUsersRepository
{
    ...
    public async Task<User?> GetUserById(Guid _Id)
        => await _context.Users.FirstOrDefaultAsync(x => x.UserId == _Id);


    public async Task<User?> GetUserByIdReadOnly(Guid _Id)
        => await _context.Users.AsNoTracking().FirstOrDefaultAsync(x => x.UserId == _Id);

    public async Task<bool> CheckUserExists(Guid _Id)
        => await _context.Users.AsNoTracking().AnyAsync(x => x.UserId == _Id);


    public async Task<string> GetUserNameByIdReadOnly(Guid _Id)
        => await _context.Users.AsNoTracking().Where(x => x.UserId == _Id).Select(x => x.UserName).FirstOrDefaultAsync() ?? string.Empty;
